...
RUN wget https://github.com/google/go-jsonnet/archive/v0.14.0.zip && \
    unzip v0.14.0.zip && mkdir -p /go/src/github.com/google/go-jsonnet && \
    mv go-jsonnet-0.14.0/* /go/src/github.com/google/go-jsonnet/ && \
...
RUN wget https://github.com/google/jsonnet/archive/v0.14.0.zip
RUN unzip v0.14.0.zip

RUN cd jsonnet-0.14.0 && \
    make -j && \
    mv jsonnet /usr/local/bin && \
    mv jsonnetfmt /usr/local/bin && \
    rm -rf /opt/jsonnet-0.14.0 && ls /usr/local/bin/jsonnet
